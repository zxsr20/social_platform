<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.youhui>" %>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../youhui/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../youhui/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            function flexiCreate() {
                ...
                window.location.href = "../youhui/Create";
                return;
            }
               //导航到修改的按钮
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../youhui/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../youhui/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../youhui/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.id) %>', name: 'id', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.name) %>', name: 'name', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.decription) %>', name: 'decription', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.net_price) %>', name: 'net_price', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.save_money) %>', name: 'save_money', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.telephone) %>', name: 'telephone', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.mobile) %>', name: 'mobile', width: 107, sortable: true, align: 'left' }
					
					, { display: '<%: Html.LabelFor(model => model.updatetime) %>', name: 'updatetime', width: 107, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.addtime) %>', name: 'addtime', width: 107, sortable: true, align: 'left' }
