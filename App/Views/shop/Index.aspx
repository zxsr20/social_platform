<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.shop>" %>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../shop/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../shop/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            ...
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../shop/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../shop/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../shop/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.id) %>', name: 'id', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.name) %>', name: 'name', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.description) %>', name: 'description', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.welcome_pic) %>', name: 'welcome_pic', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.home_pic) %>', name: 'home_pic', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.telephone) %>', name: 'telephone', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.mobile) %>', name: 'mobile', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.address) %>', name: 'address', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.fax) %>', name: 'fax', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.bus_way) %>', name: 'bus_way', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.filltime) %>', name: 'filltime', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.shop_type) %>', name: 'shop_type', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.locx) %>', name: 'locx', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.locy) %>', name: 'locy', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.userid) %>', name: 'userid', width: 68, sortable: true, align: 'left' }
