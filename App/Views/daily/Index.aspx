<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.daily>" %>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../daily/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../daily/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            ...
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../daily/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../daily/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../daily/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.id) %>', name: 'id', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.title) %>', name: 'title', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.content) %>', name: 'content', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.tasktypeOld) %>', name: 'tasktypeOld', width: 205, hide: true, sortable: false, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.tag) %>', name: 'tag', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.statusOld) %>', name: 'statusOld', width: 205, hide: true, sortable: false, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.percent) %>', name: 'percent', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.starttime) %>', name: 'starttime', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.endtime) %>', name: 'endtime', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.useridOld) %>', name: 'useridOld', width: 205, hide: true, sortable: false, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.addtime) %>', name: 'addtime', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.updatetime) %>', name: 'updatetime', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.filelink) %>', name: 'filelink', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.remark) %>', name: 'remark', width: 68, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.isdel) %>', name: 'isdel', width: 68, sortable: true, align: 'left' }
