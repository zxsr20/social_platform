<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.tasktype>" %>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../tasktype/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../tasktype/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            function flexiCreate() {
                ...
                window.location.href = "../tasktype/Create";
                return;
            }
               //导航到修改的按钮
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../tasktype/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../tasktype/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../tasktype/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.ID) %>', name: 'ID', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.tasktypename) %>', name: 'tasktypename', width: 753, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.useridOld) %>', name: 'useridOld', width: 205, hide: true, sortable: false, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.addtime) %>', name: 'addtime', width: 753, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.isdel) %>', name: 'isdel', width: 753, sortable: true, align: 'left' }
