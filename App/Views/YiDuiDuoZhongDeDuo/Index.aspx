<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.YiDuiDuoZhongDeDuo>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div id="divQuery" title="查询列表" class="easyui-dialog" closed="true" modal="false"
        style="padding: 5px; background: #fafafa;" iconCls="icon-search">
            ...
            <div class="editor-field-Search">
                <input type="text" id="ShuLiangStart_Int" onkeyup = "isInt(this)" />
                到
                <input type="text" id="ShuLiangEnd_Int" onkeyup = "isInt(this)"  />
            </div>
            <br style=" clear:both;" /> 
            <div class="input">
                <div class="editor-label">
                ...
                <div class="editor-field-Search">
                <input type='text' id='GuiGeXingHao'/>
                </div>
            </div>
            <br style="clear:both;" /> 
            <div class="editor-label">
            ...
            <div class="editor-field-Search">
                <input type="text" id="JiaoHuoQiXianStart_Time" onclick="WdatePicker()"  />
                到
                <input type="text" id="JiaoHuoQiXianEnd_Time" onclick="WdatePicker()" />
            </div>
            <br style=" clear:both;" />
    </div>
</asp:Content>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../YiDuiDuoZhongDeDuo/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../YiDuiDuoZhongDeDuo/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            function flexiCreate() {
                ...
                window.location.href = "../YiDuiDuoZhongDeDuo/Create";
                return;
            }
               //导航到修改的按钮
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../YiDuiDuoZhongDeDuo/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../YiDuiDuoZhongDeDuo/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../YiDuiDuoZhongDeDuo/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.Id) %>', name: 'Id', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.MingCheng) %>', name: 'MingCheng', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.ShuLiang) %>', name: 'ShuLiang', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.ChanPinMuLu) %>', name: 'ChanPinMuLu', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.GuiGeXingHao) %>', name: 'GuiGeXingHao', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.MyCaiGouXuQiouJiHuaIdOld) %>', name: 'MyCaiGouXuQiouJiHuaIdOld', width: 205, hide: true, sortable: false, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.JiLiangDanWei) %>', name: 'JiLiangDanWei', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.JiaoHuoQiXian) %>', name: 'JiaoHuoQiXian', width: 150, sortable: true, align: 'left' }
					, { display: '<%: Html.LabelFor(model => model.Currency) %>', name: 'Currency', width: 150, sortable: true, align: 'left' }
