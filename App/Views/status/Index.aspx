<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Index.Master" Inherits="System.Web.Mvc.ViewPage<DAL.status>" %>
...
//                            $('#divQuery').dialog("close");
            ...
            flexiOut = function (ids, grid) {
              ...
              grid.doDaoChu("../status/GetDaoChu");//导出数据
          };
          //导航到查看详细的按钮
            flexiView = function (ids, grid) {
                ...
                window.location.href = "../status/Details/" + arr[0];
                return;
                }
            }
              //导航到创建的按钮
            ...
            flexiModify = function (ids, grid) {
                ...
                if (arr.length == 1) {
                    window.location.href = "../status/Edit/" + arr[0];
                    return;
                }
            };
               //删除的按钮
            flexiDelete = function (ids, grid) {
                ...
                if (!confirm("确认删除这 " + arr.length + " 项吗？"))
                    return;
                $.post("../status/Delete", { query: arr.join(",") }, function (res) {
                    if (res == "OK") {
			...
			{
			    url: '../status/FlexigridList',//获取数据的url
			    colModel: [
				   { display: '<%: Html.LabelFor(model => model.ID) %>', name: 'ID', width: 205, hide: true, sortable: false, align: 'left' }
					...
					, { display: '<%: Html.LabelFor(model => model.statusname) %>', name: 'statusname', width: 753, sortable: true, align: 'left' }
