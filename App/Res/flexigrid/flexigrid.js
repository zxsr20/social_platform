...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        p = $.extend({
            ...
            dataType: 'json', // type of data loaded // clark
            errormsg: '连接错误，请重试...',
            usepager: false, //
            nowrap: true, //
            page: 1, //current page
            ...
            rpOptions: [10, 15, 20, 40],
            title: false,
            pagestat: '显示从 {from} 到 {to} ，共 {total} 项',
            pagetext: '第',
            outof: '页，共',
            findtext: 'Find',
            procmsg: '正在处理，请稍后...',
        ...
        var g = {
            ...
            getBindDis: function () {
                ...
                $(p.colModel).each(function (_i, _d) {
                    if ((_d.bind) == false || !_d.display) return;
                    cols.push($(_d.display).text());
                });
                return cols;
            },

            getBindNames: function () {//clark

                var cols = [];
                $(p.colModel).each(function (_i, _d) {
                    if ((_d.bind) == false || !_d.name) return;
                    cols.push(g.getName(_d.name));
                });
                return cols;
            },
            getName: function (name) { //clark 
                if ($.isFunction(name)) {
                    return name();
                }
                else return name;
            ...
            populate: function () { //get latest data
                ...
                var param = [
                   ...
                   , { name: 'cols', value: cols.join(",")} //clark  
				];
            ...
            //                this.populate();
            //            },
            doSearch: function (query) { //修改为udi的方法
            ...
            doDaoChu: function (my) { //修改为udi的方法
                var cols = g.getBindNames();    //clark 获取隐藏的列
                var names = g.getBindDis();
                var param = [
					 { name: 'page', value: p.newp }
					, { name: 'rp', value: p.rp }
					, { name: 'sortname', value: p.sortname }
					, { name: 'sortorder', value: p.sortorder }
					, { name: 'query', value: p.query }
					, { name: 'qtype', value: names.join(",") }
				   , { name: 'cols', value: cols.join(",")} //clark 获取隐藏的列
                ];
                ...
                $.ajax({
                    ...
                    url: my,
                    data: param,
                    dataType: "text", // p.dataType,  //"application/vnd.ms-excel",
                    success: function (data) {
                        var vv = window.open(data);
                        //                        var vv = window.open();
                        //                        vv.location.href = data;
                    },
                    error: function (XMLHttpRequest, textStatus, errorThrown) { try { if (p.onError) p.onError(XMLHttpRequest, textStatus, errorThrown); } catch (e) { } }
                });
            },
						...
						function () {
							...
							.click(
								function (e) {
								    ...
								    $("td:eq(1)", this).toggleClass('first');//clarkchange 选择框
            ...
            for (i = 0; i < p.buttons.length; i++) {
                ...
                if (!btn.separator) {
                    ...
                    if (btn.onpress) {
                        $(btnDiv).click
							(
                            function (ev) { //clark  将查询的数据g传输到doSearch方法
                                this.onpress(this.name, g, ev);  //clark 将查询的数据g传输到doSearch方法
                            }
                            );
                    }
                    //                    if (btn.onpress) {
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" /> ' + p.outof + ' <span> 1 </span>页</span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
        ...
        if (p == null || p == "undefined") {
            ...
            return this.each(function () {
                if (this.grid && this.p.url) {
                    ...
                    var myurl = this.p.url.lastIndexOf("FlexigridList");
                    if (myurl != null && myurl != "undefined") {
                        this.p.url = this.p.url.substring(0, myurl) + "FlexigridList/" + p;
                    }
